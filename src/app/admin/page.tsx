  ...
  const updateStatus = useCallback(
    async (orderId: string, status: string) => {
      ...
      if (data.whatsappSent) {
        setWhatsappSentMap((m) => ({ ...m, [orderId]: true }));
        ...
        {filteredOrders.map((order) => (
          ...
          >
            ...
            <div className="flex items-center gap-1.5 mb-3 text-xs">
              ...
              {order.client_profiles?.phone ? (
                <span className="text-green-600 font-medium">
                  WhatsApp{" "}
                  {whatsappSentMap[order.order_id] ? (
                    <span className="inline-flex items-center gap-0.5">✓ enviado</span>
                  ) : (
                ...
                <span className="text-red-400">
                  WhatsApp <span className="text-red-300">✕ sin número</span>
                </span>
              )}
            </div>
